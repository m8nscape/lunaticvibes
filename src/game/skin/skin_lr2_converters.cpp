...
{
    using namespace lv::data;

    auto system = [](const char* key)
    {
        if (SystemData.timers.find(key) == SystemData.timers.end())
            return TIMER_NEVER;
        return SystemData.timers.at(key);
    };

    auto select = [](const char* key)
    {
        if (SelectData.timers.find(key) == SelectData.timers.end())
            return TIMER_NEVER;
        return SelectData.timers.at(key);
    };

    auto play = [](const char* key)
    {
        if (PlayData.timers.find(key) == PlayData.timers.end())
            return TIMER_NEVER;
        return PlayData.timers.at(key);
    };

    auto result = [](const char* key)
    {
        if (ResultData.timers.find(key) == ResultData.timers.end())
            return TIMER_NEVER;
        return ResultData.timers.at(key);
    };

    auto arena = [](const char* key)
    {
        if (ArenaData.timers.find(key) == ArenaData.timers.end())
            return TIMER_NEVER;
        return ArenaData.timers.at(key);
    };
